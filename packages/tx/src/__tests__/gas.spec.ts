...
describe("getGasFeeAmount", () => {
  ...
  it("should exclude fee tokens specified in `excludedFeeMinimalDenoms`", async () => {
    const gasLimit = 1000;
    const chainId = "osmosis-1";
    const address = "osmo1...";
    const baseFee = 0.04655;
    ...
    (queryBalances as jest.Mock).mockResolvedValue({
      balances: [
        {
          /**
           * We'll exclude this token. Even though it has enough balance to pay fees, it will select uion
           */
          denom: "uosmo",
          amount: "1000000",
        },
