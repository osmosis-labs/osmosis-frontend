...
export function isInsufficientFeeError(message: string) {
  // Matches both:
  // - the canonical message thrown by AccountStore when the account has no
  //   balance at all to pay fees ("Insufficient balance for transaction fees.
  //   Please add funds to continue.")
  // - the message generated by `selectFeeAmountFromBalances` /
  //   `getFallbackFeeAmountFromBalances` when no held fee token has enough
  //   balance to cover the simulated gas ("No fee tokens found with sufficient
  //   balance on account[ for address ...]. Please add funds to continue.")
  const regexInsufficientFeeError =
    /(Insufficient balance for transaction fees\. Please add funds to continue\.|No fee tokens found with sufficient balance on account)/;
