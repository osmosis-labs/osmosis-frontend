...
it('AminoConverter', async () => {
  ...
  const back =
    AminoConverter['/osmosis.gamm.v1beta1.MsgJoinPool'].fromAmino(msg);

  expect(back).toMatchSnapshot();

  const final =
    AminoConverter['/osmosis.gamm.v1beta1.MsgJoinPool'].toAmino(back);
...
it('AminoConverter.scoped', async () => {
  const msg = osmosis.gamm.v1beta1.AminoConverter[
