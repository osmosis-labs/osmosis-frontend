...
export const AssetChart = ({
  ...
  const { data: historicalPriceData, isLoading: isHistoricalPriceDataLoading } =
  ...
  const points = useMemo<TLineChartPoint[] | undefined>(() => {
    const data = historicalPriceData?.map((point) => ({
      timestamp: point.time,
      value: dataType === "price" ? point.close : point.volume,
    }));

    if (data) pointsRef.current = data;
    return data;
...
export const ChartUnavailable: FunctionComponent = () => {
  return (
    <View style={styles.flexContainer}>
      <Text type="subtitle">No data available</Text>
    </View>
  );
};
...
export const AssetChartSkeleton = () => {
  ...
  return (
    <View style={styles.flexContainer}>
      <Animated.View style={[styles.flexColumn, animatedStyle]}>
        <Svg viewBox="0 0 700 346" fill="none" preserveAspectRatio="none">
