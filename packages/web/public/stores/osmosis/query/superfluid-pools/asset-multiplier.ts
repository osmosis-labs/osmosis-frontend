...
export class ObservableQuerySuperfluidAssetMultiplierInner extends ObservableChainQuery<SuperfluidAssetMultiplier> {
	constructor(kvStore: KVStore, chainId: string, chainGetter: ChainGetter, protected readonly denom: string) {
		super(kvStore, chainId, chainGetter, `/osmosis/superfluid/v1beta1/asset_multiplier?denom=${denom}`);
...
export class ObservableQuerySuperfluidAssetMultiplier extends ObservableChainQueryMap<SuperfluidAssetMultiplier> {
	constructor(
		...
		super(kvStore, chainId, chainGetter, denom => {
			return new ObservableQuerySuperfluidAssetMultiplierInner(this.kvStore, this.chainId, this.chainGetter, denom);
		});
	}
