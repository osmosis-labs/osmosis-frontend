...
import { formatICNSName, formatPretty, shorten } from "@osmosis-labs/utils";
...
import { MobileSessionIcon } from "~/components/assets/mobile-session-icon";
import {
  Drawer,
  DrawerButton,
  DrawerContent,
...
import { MobileSessions } from "./mobile-sessions";
...
> = observer((props) => {
  ...
  return (
    <>
      <ModalBase
        title={(() => {
          if (isMobileSessionOpen) return t("profile.mobileSessions");
          if (show1CTSettings) return "";
          return t("profile.modalTitle");
        })()}
        {...props}
        isOpen={props.isOpen}
      ...
      >
        <div className="flex flex-col items-center overflow-auto">
          <ScreenManager
            currentScreen={(() => {
              if (isMobileSessionOpen) return "mobile-sessions";
              if (show1CT && show1CTSettings) return "one-click-trading";
              return "profile";
            })()}
          >
            ...
            <Screen screenName="one-click-trading">
            ...
            <Screen screenName="mobile-sessions">
              <MobileSessions onClose={() => setIsMobileSessionOpen(false)} />
            </Screen>
          </ScreenManager>
        </div>
      </ModalBase>
