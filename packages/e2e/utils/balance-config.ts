...
> = {
  "E2E Test Account": [
    ...
    { token: "OSMO", minAmount: 1.3, warnAmount: 2.5, unit: "usd", note: "USD-denominated (MTN-157): the limit sell is fiat-mode (~$1.01), so required OSMO scales inversely with price. Fixed token thresholds went stale as OSMO fell and created a topup dead-zone (3.4 OSMO cleared the old 2.5-token gate yet starved the sells). min covers the fiat sell (~$1.01) PLUS the swap.osmo 0.2 OSMO swap (token-denominated) with ~5% buffer; warn≈2x. NB: the 0.2 OSMO swap is a fixed token need, so this USD floor under-provisions if OSMO climbs past ~$1; revisit then. NB: the check-balances auto-topup dispatches --ref stage, so the scaled target only applies once this is merged to stage." },
