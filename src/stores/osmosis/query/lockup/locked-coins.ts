...
export class ObservableQueryAccountLockedCoinsInner extends ObservableChainQuery<AccountLockedCoins> {
	constructor(kvStore: KVStore, chainId: string, chainGetter: ChainGetter, protected readonly bech32Address: string) {
		super(kvStore, chainId, chainGetter, `/osmosis/lockup/v1beta1/account_locked_coins/${bech32Address}`);
...
export class ObservableQueryAccountLockedCoins extends ObservableChainQueryMap<AccountLockedCoins> {
	constructor(kvStore: KVStore, chainId: string, chainGetter: ChainGetter) {
		super(kvStore, chainId, chainGetter, (bech32Address: string) => {
			return new ObservableQueryAccountLockedCoinsInner(this.kvStore, this.chainId, this.chainGetter, bech32Address);
		});
	}
