...
export class ObservableQueryPoolCreationFee extends ObservableChainQuery<Params> {
	constructor(kvStore: KVStore, chainId: string, chainGetter: ChainGetter) {
		super(kvStore, chainId, chainGetter, `/cosmos/params/v1beta1/params?subspace=gamm&key=PoolCreationFee`);
	...
	get poolCreationFee(): CoinPretty[] {
		...
		const value = JSON.parse(this.response.data.param.value) as { denom: string; amount: string }[];
