...
export class ObservableQueryGuageById extends ObservableChainQuery<GaugeById> {
	constructor(kvStore: KVStore, chainId: string, chainGetter: ChainGetter, id: string) {
		super(kvStore, chainId, chainGetter, `/osmosis/incentives/v1beta1/gauge_by_id/${id}`);
	}
	...
	get lockupDuration(): Duration {
		...
		if (this.response.data.gauge.distribute_to.lock_query_type !== 'ByDuration') {
			return dayjs.duration({
				seconds: 0,
			});
		}
		...
		return dayjs.duration({
			seconds: parseInt(this.response.data.gauge.distribute_to.duration.replace('s', '')),
		});
	}
	...
	get remainingEpoch(): number {
		...
		return parseInt(this.response.data.gauge.num_epochs_paid_over) - parseInt(this.response.data.gauge.filled_epochs);
	}
	...
	readonly getCoin = computedFn(
		(currency: AppCurrency): CoinPretty => {
			if (!this.response) {
				return new CoinPretty(currency, new Dec(0));
			}

			const primitive = this.response.data.gauge.coins.find(coin => coin.denom === currency.coinMinimalDenom);
			if (!primitive) {
				return new CoinPretty(currency, new Dec(0));
			}
