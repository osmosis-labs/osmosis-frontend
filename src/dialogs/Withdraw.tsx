...
// TODO : is almost code as deposit - merge them together
export const WithdrawDialog: FunctionComponent<BaseModalProps & { token: string }> = observer(
	({ isOpen, close, style, token }) => {
		// TODO : @Thunnini fetch addresses
		const from = 'osmo1ymk637a7wljvt4w7q9lnrw95mg9sr37y4s4an9';
		const to = 'cosmos1ymk637a7wljvt4w7q9lnrw95mg9sr37y4s4an9';
		const available = 3502.350215;
		const [input, setInput] = React.useState('');

		const onDeposit = React.useCallback(
			input => {
				if (!input || input === '0') return alert('Please input an amount to withdraw');
				if (gt(input, available)) return alert('Not enough available balance');
				alert('Successfully requested withdraw');
				close();
			},
			[available, close]
		);
		...
		return (
			<FocusedBaseDialog style={style} isOpen={isOpen} close={close}>
				<div className="w-full h-full text-white-high">
					...
					<h6 className="mt-7">Amount To Withdraw</h6>
