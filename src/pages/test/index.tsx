...
export const TestPage: FunctionComponent = observer(() => {
	...
	return (
		<div>
			...
			<div>Balances</div>
			{queries.queryBalances.getQueryBech32Address(accountInfo.bech32Address).balances.map(bal => {
				return <div key={bal.currency.coinMinimalDenom}>{bal.balance.toString()}</div>;
			})}
			<div>Pools</div>
