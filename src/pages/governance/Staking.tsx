		...
		<table className="w-full h-full">
			...
			<tbody className="w-full h-full">
				{map(stakingData, (data, i) => (
					<StakingRow key={i} data={data} height={72} />
				))}
			</tbody>
		</table>
	);
...
const StakingRow: FunctionComponent<{ data: IStakedValidator; height: number }> = ({ data, height }) => {
	let i = 0;
	return (
		<tr style={{ height: `${height}px` }} className="px-5 w-full flex items-center border-b border-white-faint">
			...
			<td className="pl-4 flex items-center justify-end" style={{ width: ROW_WIDTHS[i++] }}>
				<button onClick={() => alert('Manage Flow')} className="hover:opacity-75 cursor-pointer flex items-center">
